...
Module({
    ...
    try {
        ...
        await message.sendReply('_Processing video for UHD snapshots..._');
        ...
        if (!response.ok) {
            ...
            throw new Error(`Processing service returned HTTP ${response.status}: ${errorText.slice(0, 200)}`);
        }

        const result = await response.json();
        if (result.type !== 'images' || !Array.isArray(result.images) || !result.images.length) {
            throw new Error('Processing service returned no snapshots');
        }
        ...
        for (let index = 0; index < result.images.length; index += 1) {
            ...
            await message.sendMessage(Buffer.from(item.data, 'base64'), 'image', {
                ...
                caption: index === 0 ? '_UHD snapshots_' : undefined
            });
        }
    } catch (error) {
        console.error('snapshot plugin error:', error);
        await message.sendReply(`_Could not extract photos from that video._\n\n\`${String(error.message || error).slice(0, 250)}\``);
    }
});
